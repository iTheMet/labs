...
public class Calculation {
    ...
    public static void Calc(){
      ...
      /*for(int i=1; i<=2;i++) {
          Ef = 1 / Tok;
          Tok = KapVlojAvtInf / EkonomEffect;
          KapVlojAvtInf = KapVlojNaProekt + KapVlojNaReal;
          KapVlojNaProekt = ((1 + Wd) * (1 + Wc) + Wh) * (Zo1 + Zo2) + Sm + Mv;
          Sm = 5;//SQL PARSING LOL
          Mv = Tmv * Smhours * Kmultiprogramm;
          KapVlojNaReal = 434346;// Уёбища из Томска не сказали как норм расчитать эту парашу
          EkonomEffect = Z1 * Ak * Z2;//ЭТО Э
          Ztek1 = Zzp1 + Sa1 + Ze1 + Srem1 + Zm + Zn1;
          Ztek2 = Zzp2 + Sa2 + Ze2 + Srem2 + Zm + Zn2;
          Ak = Jeta1 / Jeta2;//Соси из SQL
          Jeta1 = 1.4;
          Jeta2 = 1.9;
      }
...
*/
        ...
        Ztek2 = Zzp2 + Sa2 + Ze2 + Srem2 + Zm + Zn2;
        EkonomEffect = Z1 * Ak * Z2;//ЭТО Э
        KapVlojNaReal = 16875;// Уёбища из Томска не сказали как норм расчитать эту парашу
        Sm = 5;//SQL PARSING LOL
        Mv = Tmv * Smhours * Kmultiprogramm;
        KapVlojNaProekt = ((1 + Wd) * (1 + Wc) + Wh) * (Zo1 + Zo2) + Sm + Mv;
        KapVlojAvtInf = KapVlojNaProekt + KapVlojNaReal;
            EkonomEffect = 126757.06;//TEST
            KapVlojAvtInf = 161798.09;//TEST
        Tok = KapVlojAvtInf / EkonomEffect;
        Ef = (1 / Tok);
        System.out.println("Ef= "+Ef+"\nTok= "+Tok+"\nKapVlojAvtInf= "+KapVlojAvtInf+"\n" +
                "EkonomEffect= "+EkonomEffect+"\nKp="+ KapVlojNaProekt+"\n" +
